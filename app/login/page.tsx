...
export default function LoginPage() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    setIsLoading(true)
    try {
      const response = await fetch("http://127.0.0.1:8080/api/v1/auth/sign-in", {
        method: "POST",
        headers: {
      ...
      // Backend-dən gələn xətanı tuturuq
      if (!response.ok || (data.code && data.code !== 200)) {
        throw new Error(data.message || "Giriş zamanı xəta baş verdi")
      }
      
      // Token yoxdursa, deməli giriş uğursuzdur
      ...
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-background text-foreground overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          <div className="relative flex h-32 w-32 items-center justify-center mb-8">
            ...
            <Gamepad2 className="h-16 w-16 text-neon animate-bounce" />
          </div>
      ...
      <Card className="relative z-10 w-full max-w-md border-neon/30 bg-card/40 backdrop-blur-xl shadow-[0_0_40px_-10px_rgba(var(--neon),0.3)] transition-all duration-500 hover:shadow-[0_0_60px_-10px_rgba(var(--neon),0.4)] hover:border-neon/50">
        <CardHeader className="space-y-1 text-center pb-2">
          ...
          <CardTitle className="text-3xl font-bold tracking-tight text-foreground font-mono">
            GAMING<span className="text-neon">CAFE</span>
          </CardTitle>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-foreground/80 text-xs uppercase tracking-wider font-semibold">Email Access</FormLabel>
                    <FormControl>
                      <div className="relative group">
                        <Input 
                          placeholder="admin@gamingcafe.com"
                          {...field} 
                          className="bg-secondary/30 border-border/50 focus:border-neon/70 focus:ring-neon/20 transition-all duration-300 h-11 pl-4 font-mono text-sm group-hover:border-neon/30"
                        />
                        <div className="absolute inset-0 rounded-md ring-1 ring-inset ring-neon/0 transition-all duration-300 pointer-events-none group-hover:ring-neon/10"></div>
                      </div>
                    </FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-foreground/80 text-xs uppercase tracking-wider font-semibold">Security Key</FormLabel>
                    <FormControl>
                      <Input 
                        type="password" 
                        {...field} 
